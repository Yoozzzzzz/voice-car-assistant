...
| D13 | 服务启动管控 | **用户没有明确要求时，AI 不得自行启动任何服务**（server `npm run dev`、client `expo start`、任何端口监听进程等）。需要运行服务验证时，先向用户说明意图并获得明确同意；AI 曾启动的服务在验证完毕或用户要求时必须彻底停止（含 watch 父进程，确认端口释放） |
...
| X7 | 汇报规范 | 完成工作后向用户如实汇报：实现了什么功能，或修复了什么 bug（含现象描述：出错的场景/报错信息/修复前后对比）。禁止无信息量的客套话与自我评价式废话 |
...
### 2026-09-23（续）— 停止 dev 服务 + 新增 D13 服务启动管控门禁
- 用户指令：停掉 AI 启动的 8080 服务；并在规范中增加"用户没有明确要求开启服务时，不能自己启动服务"
- **服务已彻底停止**：杀掉 8080 监听进程（PID 23604）后检测到 tsx watch 父进程（PID 24444）仍存活（有守护重启风险），一并停止；最终确认 8080 端口释放、无 server 相关 node 进程残留。client 的 jest-worker/Metro 进程属用户自身 Expo 会话，未动
...
3. **开发中**：遵守"开发过程门禁"13 项约束（注意 **D7 不写 mock、D11 必须用 context7 核实库/API（不可用时 fallback web 搜索并记日志）、D12 Key 安全、D13 用户未明确要求不得自行启动服务**）
